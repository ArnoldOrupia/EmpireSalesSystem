...
public class Salaries extends JFrame{
    ...
    setSize(1200,1200);
       ...
       // table.setBounds(0, 50, 1190, 1100);
        ...
        label1 =new JLabel("THE FOLLOWING IS THE SUMMARY EARNINGS FOR EACH EMPLOYEE");
    ...
    protected static JTable createView() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT username, hotpots,flasks,glasses,(hotpots+flasks+glasses) as Revenue,"
                    + "(0.01 * (hotpots+flasks+glasses)) as salary, (0.01 * (0.01 * (hotpots+flasks+glasses))) AS BONUS ,"
                    + "((0.01 * (0.01 * (hotpots+flasks+glasses)))+ (0.01 * (hotpots+flasks+glasses))) AS TotalEarnings FROM SALARIES ORDER BY TotalEarnings ");
